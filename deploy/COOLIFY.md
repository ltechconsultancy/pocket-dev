...
| **`node-compile-cache: file exists`** | Coolify cannot delete volumes in UI (read-only). Redeploy latest `main`: compose uses new volume `shared-tmp-v2` + `tmpfs` for `/tmp`. Or stop app, edit compose, reload, redeploy. |
